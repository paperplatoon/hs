...
import { canPlayCard, playCardFromHand, declareAttack, attackTarget, endTurn, canUseHeroPower, useHeroPower, getEffectiveCost } from '../engine/actions.js';
...
import { animatedAttack, animatedPlayCard, animatedEnemyTurn, animatedEndTurn, animatedHeroPower } from '../animations/combat.js';
...
function renderHeroAvatar(state, playerId, setState, animateAttack) {
  ...
  portrait.textContent = isEnemy ? (player.name || 'Enemy') : 'You';

  // Hero power button with custom tooltip
  const heroPowerBtn = document.createElement('button');
  heroPowerBtn.className = 'btn btn--power';

  const heroPowerDef = player.heroPower ? HERO_POWERS[player.heroPower] : null;
  if (heroPowerDef) {
    // Build button content with tooltip
    heroPowerBtn.innerHTML = `
      <span class="power-name">${heroPowerDef.name}</span>
      <span class="power-cost">${heroPowerDef.cost}</span>
      <span class="power-tooltip">${heroPowerDef.text}</span>
    `;

    // Only player's hero power is interactive
    if (!isEnemy) {
      const canUse = canUseHeroPower(state, playerId);
      heroPowerBtn.disabled = !canUse;
      heroPowerBtn.classList.toggle('usable', canUse);
      heroPowerBtn.classList.toggle('used', player.heroPowerUsed);

      if (canUse) {
        heroPowerBtn.addEventListener('click', () => {
          animatedHeroPower(state, setState);
        });
      }
    } else {
      // Enemy hero power - just show info
      heroPowerBtn.disabled = true;
      heroPowerBtn.classList.toggle('used', player.heroPowerUsed);
    }
  } else {
    heroPowerBtn.textContent = 'No Power';
    heroPowerBtn.disabled = true;
  }

  avatar.append(portrait, health, mana, heroPowerBtn);
...
function renderHand(el, state, setState) {
  ...
  p.hand.forEach((c, i) => {
    ...
    if (playable) {
      ...
      card.addEventListener('click', () => animatedPlayCard(state, Players.PLAYER, i, setState));
    }
    el.append(card);
  });
}
...
function renderCardFooter(cardInstance) {
  const def = CARDS[cardInstance.cardId];
  const tier = cardInstance.tier || def?.tier || 1;
  const evolveInfo = cardInstance.keywords?.evolve || def?.keywords?.evolve;
  ...
  // Evolve preview (if card has evolve keyword)
  if (evolveInfo?.cardId) {
    ...
    if (evolveDef) {
      ...
      // Hover popup
      evolvePreview.addEventListener('mouseenter', (e) => {
        ...
        const evolveInstance = createMinionInstance(evolveDef);
        const popupCard = renderMinionCard(evolveInstance, null, null, null, null);
        ...
        // If popup goes off-screen top, show below instead
...
function renderMinionCard(m, state, owner, zone, index) {
  ...
  card.append(renderCardFooter(m));
  wrap.append(card);
...
function renderHandCard(c, state, owner, zone, index) {
  ...
  card.append(portrait);
  card.append(renderCardFooter(c));
  wrap.append(card);

  // Cost orb
  const baseCost = c.cost ?? c.stats?.cost ?? 0;
  const costVal = (owner && state) ? getEffectiveCost(state, owner, c) : baseCost;
  const cost = document.createElement('div');
  cost.className = 'cm-orb cost' + (costVal < baseCost ? ' discounted' : '');
