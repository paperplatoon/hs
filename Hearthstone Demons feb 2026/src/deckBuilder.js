...
import { CARDS, createMinionInstance } from './cards/schema.js';
...
// Get available cards for gallery (collectible cards only)
export function getAvailableCards() {
  return Object.keys(CARDS).filter(id => CARDS[id].collectible !== false);
}
...
function renderDeckBuilderFooter(cardDef) {
...
export function renderFullCard(cardId, options = {}) {
  ...
  card.appendChild(renderDeckBuilderFooter(cardDef));
