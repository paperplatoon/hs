...
import { ENEMIES } from './enemies/schema.js';
...
export function createInitialState() {
  return {
    // Screen management
    currentScreen: 'start-menu', // 'start-menu' | 'deck-builder' | 'combat' | 'shop' | 'remove-card'
    ...
    run: {
      ...
      opponentDeck: [], // Array of card IDs for current opponent
      currentEnemy: null, // Enemy ID from enemies/schema.js (e.g., 'enemy:goblin-horde')
    },
...
function createPlayerState(id) {
  return {
    ...
    graveyard: [],
    character: null, // Character ID (e.g., 'char:paladin')
    heroPower: null, // Hero power ID (e.g., 'power:small-shield')
    heroPowerUsed: false, // Reset each turn
    effects: {
      multipliers: {
        growing: 1,
      },
...
// Initialize a run with a player character (enemy selected randomly at combat start)
export function initializeRunWithPlayerCharacter(state, playerCharacter) {
  return withState(state, (ns) => {
    // Set player character and hero power
    ns.players[Players.PLAYER].character = playerCharacter.id;
    ns.players[Players.PLAYER].heroPower = playerCharacter.heroPower;
    ns.run.playerDeck = playerCharacter.starterDeck.slice(); // Copy deck IDs

    // Clear opponent - will be selected randomly in startGame
    ns.run.opponentDeck = [];
    ns.run.currentEnemy = null;
...
// Initialize a run with both characters (for specific matchups)
export function initializeRunWithCharacter(state, playerCharacter, opponentCharacter) {
  return withState(state, (ns) => {
    // Set player character and hero power
    ns.players[Players.PLAYER].character = playerCharacter.id;
    ns.players[Players.PLAYER].heroPower = playerCharacter.heroPower;
    ...
    // Set opponent character and hero power
    ns.players[Players.OPPONENT].character = opponentCharacter.id;
    ns.players[Players.OPPONENT].heroPower = opponentCharacter.heroPower;

    // Store opponent deck in a parallel field (used by startGame)
    ns.run.opponentDeck = opponentCharacter.starterDeck.slice();
...
export function resetCombatState(state) {
  return withState(state, (ns) => {
    ...
    // Reset player states (preserve character and heroPower)
    [Players.PLAYER, Players.OPPONENT].forEach(playerId => {
      ...
      ns.players[playerId].heroPowerUsed = false;
      // character and heroPower are preserved (set at run start)
    });
...
export function onCombatVictory(state) {
  return withState(state, (ns) => {
    // Get gold reward from enemy, default to 50
    const enemy = ns.run.currentEnemy ? ENEMIES[ns.run.currentEnemy] : null;
    const goldReward = enemy?.goldReward || 50;

    ns.run.gold += goldReward;
    ns.run.round += 1;
    ns.currentScreen = 'shop';
    // Clear shop inventory so it regenerates with new cards
    ns.shop.availableCards = [];
    // Clear current enemy for next fight
    ns.run.currentEnemy = null;
    ns.run.opponentDeck = [];
    pushLog(ns, `Victory! Earned ${goldReward} gold. Total: ${ns.run.gold}`);
  });
}
