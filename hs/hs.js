...
async function startEncounter(stateObj) {
  stateObj = immer.produce(stateObj, (newState) => {
    ...
    (newState.opponent.encounterDraw = [...potentialEnemies[0].deck]),
      (newState.opponent.heroPower = {
        ...heroPowers[potentialEnemies[0].heroPower],
      });
...
async function completeAttack(
  stateObj,
  attackerIndex = stateObj.playerToAttackIndex,
  defenderIndex = stateObj.enemyToBeAttackedIndex
) {
  if (defenderIndex === 99) {
    ...
    document
      .querySelectorAll("#playerMonstersInPlay .avatar")
      [attackerIndex].classList.add("attack-windup");
    await pause(300);
    document
      .querySelectorAll("#enemyMonstersInPlay .avatar")
      [defenderIndex].classList.add("attack-impact");
    document
      .querySelectorAll("#playerMonstersInPlay .attack")
      [attackerIndex].classList.add("attack-bulge");
    await pause(200);
    stateObj = immer.produce(stateObj, (newState) => {
      let AttackingMonster = newState.player.monstersInPlay[attackerIndex];
      ...
      console.log(
        "Player's " +
          ...
          AttackingMonster.attack +
          " damage to enemy's " +
          DefendingMonster.name +
          " and took " +
          DefendingMonster.attack +
          " damage"
      );
    ...
    document
      .querySelectorAll("#enemyMonstersInPlay .avatar")
      [defenderIndex].classList.remove("attack-impact");
    document
      .querySelectorAll("#playerMonstersInPlay .avatar")
      [attackerIndex].classList.remove("attack-windup");
    document
      .querySelectorAll("#playerMonstersInPlay .attack")
      [attackerIndex].classList.remove("attack-bulge");
...
async function playDemonFromHand(
  stateObj,
  cardObj,
  playerSummoning,
  pauseTime = 1000,
  ...
  stateObj = await immer.produce(stateObj, async (newState) => {
    let player =
      playerSummoning.name === "player" ? newState.player : newState.opponent;
    player.currentEnergy -= cardObj.baseCost;
  });

  stateObj = await summonDemon(
    stateObj,
    cardObj,
    playerSummoning,
    pauseTime,
...
async function summonDemon(
  ...
  if (pushToEnd) {
    let queryString =
      playerSummoning.name === "player"
        ? "#playerMonstersInPlay .avatar"
        : "#enemyMonstersInPlay .avatar";
    let cardString =
      playerSummoning.name === "player"
        ? "#playerMonstersInPlay .card"
        : "#enemyMonstersInPlay .card";
    let monstersLength =
      playerSummoning.name === "player"
        ? stateObj.player.monstersInPlay.length
        : stateObj.opponent.monstersInPlay.length;
    ...
    document
      .querySelectorAll(cardString)
      [monstersLength - 1].classList.add("opaque");
    document
      .querySelectorAll(queryString)
      [monstersLength - 1].classList.add("summon-demon");
    await pause(500);
    document
      .querySelectorAll(cardString)
      [monstersLength - 1].classList.remove("opaque");
    await pause(600);
    document
      .querySelectorAll(queryString)
      [monstersLength - 1].classList.remove("summon-demon");
  }
...
async function createNewMinion(
  stateObj,
  playerSummoning,
  attack = 1,
  currentHealth = 1,
...
async function gainLife(stateObj, playerSummoning, lifeToGain) {
  ...
  if (
    playerSummoning.name === "player" &&
    playerSummoning.quest.title === "Gain Life"
  ) {
    stateObj = immer.produce(stateObj, (newState) => {
...
async function giveDemonStats(
  ...
  stateObj = immer.produce(stateObj, (newState) => {
    let player =
      playerObj.name === "player" ? newState.player : newState.opponent;
    ...
    if (stat2Name) {
      if (stat2Name === "maxHealth") {
        ...
        stat2Value =
          missingHP > array[index].currentHealth
            ? 0
            : array[index].currentHealth - missingHP;
      }
...
async function healMinion(stateObj, playerSummoning, index, HPToHeal) {
  stateObj = immer.produce(stateObj, (newState) => {
    ...
    let missingHP =
      monstersArray[index].maxHealth - monstersArray[index].currentHealth;
      ...
      for (let i = 0; i < indexesToDelete.length; i++) {
        let cards =
          playerObj.name === "player"
            ? document.querySelectorAll("#playerMonstersInPlay .card")
            : document.querySelectorAll("#enemyMonstersInPlay .card");
        let avString =
          playerObj.name === "player"
            ? document.querySelectorAll("#playerMonstersInPlay .avatar")
            : document.querySelectorAll("#enemyMonstersInPlay .avatar");
        cards[indexesToDelete[i]].classList.add("opaque");
        avString[indexesToDelete[i]].classList.add("avatar-death");
        await pause(800);
        let monsterObj =
          stateObj[playerObj.name].monstersInPlay[indexesToDelete[i]];
        if (typeof monsterObj.onDeath === "function") {
          let mult = stateObj[playerObj.name].onDeathMultiplier;
          for (let m = 0; m < mult; m++) {
            stateObj = await stateObj[playerObj.name].monstersInPlay[
              indexesToDelete[i]
            ].onDeath(
              stateObj,
              indexesToDelete[i],
              playerObj.monstersInPlay,
              playerObj
            );
          }
        }
...
async function changeStatus(
  stateObj,
  newStatus,
  countsAsEventSkipForChangeStatus = false,
  skipGoldGift = 50
) {
  stateObj = immer.produce(stateObj, (newState) => {
...
function createConjurerTrickButton(stateObj, playerObj) {
  ...
  ConjurerTrickButton.textContent =
    `{${stateObj[playerObj.name].heroPower.cost(
      stateObj,
      stateObj[playerObj.name]
    )} mana}: ` + stateObj[playerObj.name].heroPower.text(stateObj, playerObj);
  if (playerObj.name === "player") {
    if (
      stateObj.canPlay === true &&
      stateObj.player.currentEnergy >=
        stateObj.player.heroPower.cost(stateObj, playerObj)
    ) {
...
function createHandEndDiv(stateObj, playerObj) {
  let trickButton = createConjurerTrickButton(
    stateObj,
    stateObj[playerObj.name]
  );
  ...
  if (playerObj.name === "player") {
    if (playerObj.quest !== false) {
      ...
      questText.textContent = stateObj.player.quest.text(
        stateObj,
        stateObj.player
      );
...
function renderPlayerMonstersInPlay(stateObj) {
  ...
  if (stateObj.player.monstersInPlay.length > 0) {
    stateObj.player.monstersInPlay.forEach(async function (cardObj, index) {
      await renderCard(
        stateObj,
        stateObj.player.monstersInPlay,
        index,
        stateObj.player,
...
function renderEnemyMonstersInPlay(stateObj) {
  ...
  if (stateObj.opponent.monstersInPlay.length > 0) {
    stateObj.opponent.monstersInPlay.forEach(function (cardObj, index) {
      renderCard(
        stateObj,
        stateObj.opponent.monstersInPlay,
        index,
        stateObj.opponent,
        "#enemyMonstersInPlay",
        (functionToAdd = false)
...
async function renderLostFight(stateObj) {
  document.getElementById("app").innerHTML =
    "<p>You lost the fight!</p> <button onClick=changeState(gameStartState)> Click me to retry</button>";
}

async function renderWonFight(stateObj) {
  document.getElementById("app").innerHTML =
    "<p>You won the fight!</p> <button onClick=changeState(gameStartState)> Click me to retry</button>";
}
...
async function renderCard(
  ...
  cardDefendDiv.textContent =
    cardObj.currentHealth > 0 ? cardObj.currentHealth : 0;
  ...
  if (cardArray === stateObj.player.encounterHand) {
    if (
      cardObj.minReq(stateObj, index, stateObj.player.encounterHand) <=
      stateObj.player.currentEnergy
    ) {
      if (stateObj.canPlay === true) {
    ...
    cardArray === stateObj.opponent.monstersInPlay &&
    stateObj.playerToAttackIndex !== false
  ) {
    ...
    cardArray === stateObj.player.monstersInPlay &&
    cardArray[index].canAttack === true
  ) {
    if (stateObj.canPlay === true) {
      ...
      enemyHealthDiv.addEventListener("dragover", (event) => {
        console.log("drag hand end");
        event.preventDefault();
      });

      enemyHealthDiv.addEventListener("drop", (event) => {
        console.log("drop hand end");
        //const cardId = event.dataTransfer.getData("text/plain");
      ...
      enemyMonsters.addEventListener("dragover", (event) => {
        console.log("drag monsters in play");
        let droppableElements = document.querySelectorAll(
          "#enemyMonstersInPlay .card"
        );
  ...
  if (
    cardArray === stateObj.player.monstersInPlay &&
    stateObj.playerToAttackIndex === index
  ) {
  ...
  if (stateObj.status === Status.chooseEnemyMonster) {
    if (cardArray !== stateObj.opponent.monstersInPlay) {
      if (
        cardArray !== stateObj.player.monstersInPlay ||
        index !== stateObj.playerToAttackIndex
      ) {
...
async function PlayACardImmer(stateObj, cardObj, cardIndexInHand, playerObj) {
  ...
  if (cardObj.cardType === "minion") {
    stateObj = await playDemonFromHand(
      stateObj,
      cardObj,
      playerObj,
      (pauseTime = cardObj.pauseTime ? cardObj.pauseTime : 1000),
      (stateChange = true)
    );
  }
...
async function playACard(stateObj, cardIndexInHand, arrayObj, playerObj) {
  ...
  stateObj = await PlayACardImmer(
    stateObj,
    stateObj[playerObj.name].encounterHand[cardIndexInHand],
    cardIndexInHand,
    playerObj
  );
  ...
  for (let i = 0; i < mult; i++) {
    stateObj = newMonsters[newMonsters.length - 1].action
      ? await newMonsters[newMonsters.length - 1].action(
          stateObj,
          newMonsters.length - 1,
          newMonsters,
          playerObj
        )
      : stateObj;
...
function chooseThisMonster(stateObj, index) {
  stateObj = immer.produce(stateObj, (newState) => {
    ...
    (newState.opponent.encounterDraw = [...potentialEnemies[i].deck]),
      (newState.opponent.heroPower = {
        ...heroPowers[potentialEnemies[i].heroPower],
      });
  });
...
async function renderClickableCardList(
  stateObj,
  cardArray,
  divName,
  functionToAdd
...
function shuffleDraw(stateObj, playerShuffling) {
  stateObj = immer.produce(stateObj, (newState) => {
    newState[playerShuffling.name].encounterDraw = shuffleArray(
      stateObj[playerShuffling.name].encounterDraw
    );
  });
  return stateObj;
}
...
async function endTurn(stateObj) {
  ...
  for (let i = stateObj.player.monstersInPlay.length - 1; i > -1; i--) {
    ...
    if (
      typeof card.endOfTurn === "function" &&
      stateObj.status === Status.inFight
    ) {
      await executeAbility("player", i);
      for (let j = 0; j < stateObj.player.endofTurnMultiplier; j++) {
        stateObj = await card.endOfTurn(
          stateObj,
          i,
          stateObj.player.monstersInPlay,
          stateObj.player
...
async function attackAnimation(
  ...
  let hexString =
    attackerName === "player"
      ? "#playerMonstersInPlay .attack"
      : "#enemyMonstersInPlay .attack";
  document
    .querySelectorAll(attackerString)
    [attackerIndex].classList.add("attack-windup");
  await pause(300);
  ...
  let targetString =
    targetName === "player"
      ...
      : "#enemyMonstersInPlay .avatar";
  document
    .querySelectorAll(targetString)
    [targetIndex].classList.add("attack-impact");
  document
    .querySelectorAll(hexString)
    [attackerIndex].classList.add("attack-bulge");
  await pause(400);
  document
    .querySelectorAll(targetString)
    [targetIndex].classList.remove("attack-impact");
  document
    .querySelectorAll(attackerString)
    [attackerIndex].classList.remove("attack-windup");
}
...
async function addImpact(playerName, index) {
  if (index !== "health") {
    ...
    document
      .querySelectorAll(queryString)
      [index].classList.add("opponent-windup");
    await pause(400);
    document
      .querySelectorAll(queryString)
      [index].classList.remove("opponent-windup");
  } else {
    let queryString =
      playerName === "player" ? "#player-health-div" : "#opponent-health-div";
...
async function executeAbility(playerName, index) {
  ...
  document
    .querySelectorAll(queryString)
    [index].classList.add("execute-ability");
  await pause(500);
  document
    .querySelectorAll(queryString)
    [index].classList.remove("execute-ability");
}
...
async function endTurnIncrement(stateObj) {
  for (let i = 0; i < stateObj.opponent.monstersInPlay.length; i++) {
    if (stateObj.opponent.monstersInPlay[i].canAttack === true) {
      if (stateObj.player.monstersInPlay.length > 0) {
        let playerTargetIndex = Math.floor(
          Math.random() * stateObj.player.monstersInPlay.length
        );
        console.log(
          stateObj.opponent.monstersInPlay[i].name +
            " deals " +
            stateObj.opponent.monstersInPlay[i].attack +
            " damage to " +
            stateObj.player.monstersInPlay[playerTargetIndex].name
        );
        stateObj = await immer.produce(stateObj, async (newState) => {
          newState.player.monstersInPlay[playerTargetIndex].currentHealth -=
            newState.opponent.monstersInPlay[i].attack;
          newState.opponent.monstersInPlay[i].currentHealth -=
            newState.player.monstersInPlay[playerTargetIndex].attack;
        });
...
async function enemyTurn(stateObj) {
  ...
  if (
    stateObj.opponent.heroPower.priority > 0 &&
    stateObj.opponent.currentEnergy >=
      stateObj.opponent.heroPower.cost(stateObj, stateObj.opponent)
  ) {
    console.log("opponent used their hero power");
    stateObj = await stateObj.opponent.heroPower.action(
      stateObj,
      stateObj.opponent
    );
    usedHeroPower = true;
  ...
  for (let i = 0; i < stateObj.opponent.encounterHand.length; i++) {
    if (currentEnergy > 0) {
      ...
      if (
        cardObj.cost(stateObj, i, stateObj.opponent.encounterHand) <=
        currentEnergy
      ) {
        currentEnergy -= cardObj.cost(
          stateObj,
          i,
          stateObj.opponent.encounterHand
        );
  ...
  if (indexesToDelete.length > 0) {
    ...
    for (let i = 0; i < indexesToDelete.length; i++) {
      ...
      stateObj = cardObj.action
        ? await cardObj.action(
            stateObj,
            indexesToDelete[i],
            stateObj.opponent.encounterHand,
            stateObj.opponent
  ...
  for (let i = 0; i < stateObj.opponent.monstersInPlay.length; i++) {
    if (stateObj.opponent.monstersInPlay[i].endOfTurn) {
      stateObj = await stateObj.opponent.monstersInPlay[i].endOfTurn(
        stateObj,
        i,
        stateObj.opponent.monstersInPlay,
        stateObj.opponent
...
async function drawACard(stateObj, playerDrawing) {
  ...
  stateObj = immer.produce(stateObj, (newState) => {
    let chosenPlayer =
      playerDrawing.name === "player" ? newState.player : newState.opponent;
  ...
  if (
    playerDrawing.name === "player" &&
    playerDrawing.quest.title === "Draw Cards"
  ) {
    stateObj = immer.produce(stateObj, (newState) => {
